...
const mobileEvents = [
  ...
  { date: '2024 . 05 . 25', title: 'Awareness meeting' },
...
const sortedMobileEvents = mobileEvents.sort((a, b) => parseDate(a.date) - parseDate(b.date));
        ...
        <div className="w-full max-w-lg">
          {sortedMobileEvents.map((event, index) => (
            <div key={index} className="relative mb-8 flex flex-col items-center">
              ...
              <div className="p-4 bg-purple-500 text-black rounded-lg text-center w-full">
                <p className="text-sm">{event.date}</p>
                <p className="font-bold">{event.title}</p>
              </div>
              {index < sortedMobileEvents.length - 1 && (
                <div className="w-1 h-8 bg-pink-500 mt-2"></div>
              )}
            </div>
          ))}
        </div>
